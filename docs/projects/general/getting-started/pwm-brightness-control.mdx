...
Open the project `PWMBrightnessControl` in the folder [`MadExamples`](https://github.com/madmachineio/MadExamples)/`Examples`/`GettingStarted` if you downloaded the examples.

```swift
// Brighten or dim the LED by changing the duty cycle of a PWM signal.
...
// Initialize a variable to store the value of the duty cycle.
var value: Float = 0.0
