...
:::note
All projects in the section **General** will use SwiftIO Feather as an example. You can also use the SwiftIO board instead. If so, you need to [change the board type](../../../overview/advanced/change-board) and code (if necessary) accordingly.
:::
...
Open the project `Blink` in the folder [`MadExamples`](https://github.com/madmachineio/MadExamples)/`Examples`/`GettingStarted` if you downloaded the folder.
