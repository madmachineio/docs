...
Open the project `LEDsBrightnessControl` in the folder [`MadExamples`](https://github.com/madmachineio/MadExamples)/`Examples`/`SimpleIO` if you downloaded the examples.

```swift
// Make 3 LEDs to brighten and dim one by one.
