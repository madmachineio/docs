...
Open the project `ButtoncontrolLED` in the folder [`MadExamples`](https://github.com/madmachineio/MadExamples)/`Examples`/`SimpleIO` if you downloaded the examples.
...
while true {
    // Check the state of the button. If it is pressed, the value will be true and then turn off the LED.
