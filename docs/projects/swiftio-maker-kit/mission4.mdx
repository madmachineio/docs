...
// Initialize the analog pin that the pot connects.
...
while true {
    // Alternate states of the output signal.
    led.toggle()

    // Return the percentage of the input voltage in the range of 0.0 to 1.0.
    let analogValue = a0.readPercent()
    
    // Stop the program for a certain period based on the value to change the blink speed.
