...
You have learned some digital stuff, now let’s look into analog signals. You'll connect a potentiometer to your circuit the read analog values.
...
As mentioned before, the signals can be divided into two types: digital signal and analog signal. You have learned a lot about digital signals and tried many projects with digital devices. Now let’s know something about analog signals.

Unlike the digital signal changing suddenly from one level to another, the analog signal is **continuous**. Its voltages change smoothly with time, which means there are infinite possible values from minimum to maximum voltage (0 - 3.3V). 
...
The ability to measurement analog values depends on the ADC resolution. The **resolution** tells the count of possible values. For example, if the resolution is 2-bit, there can be 4 possible values: 0, 1, 2, 3. These values are called **raw values**. And if it's is 3-bit, there are 8 possible values (0-7).
...
As you can see, higher resolution means more possible values, so the voltage obtained will be more accurate.
...
**Potentiometer** (pot for short, also called knob) is a kind of variable resistor. You can rotate it to change its resistance and thus change the voltage in a circuit. It is commonly used to control the volume on audio devices, as a menu control on home appliances, etc. 

A potentiometer has a movable wiper inside it. As you rotate it clockwise or anticlockwise, the wiper moves to change the available resistance in your circuit. 
...
Since the two resistors are in series, the current flows through them are the same. So the ratio of R1’s resistance to the total resistance equals the ratio of voltage drop to the incoming voltage supply.
...
## 🔸Exercise
1. [Read input value](./potentiometer#1-read-input-value)
2. [Control buzzer sound output](./potentiometer#2-control-buzzer-sound-output)
3. [Change LED brightness](./potentiometer#3-change-led-brightness)
...
1. Click the icon as shown below to open the tool. 
  ...
  It will show all available ports. The second one is the one needed. The port name may be different for your board. If you are not sure which is the right one, you can disconnect your board and then connect again to see which port disappears and reappears.

  <img
    src={require('./img/serial.png').default}
    alt="Serial tool" width="960"
  />
...
2. Click the port to connect it. As you see, the red dot next to the port name changes to green. The port is successfully connected.
  ...
  <img
    src={require('./img/connectPort.png').default}
    alt="Connect the serial port" width="960"
  />
...
3. Check the serial setting. Click the right caret to show the settings. Set the **Baud Rate to 115200**. 
  ...
  <img
    src={require('./img/baudrate.png').default}
    alt="Set the serial" width="960"
  />
