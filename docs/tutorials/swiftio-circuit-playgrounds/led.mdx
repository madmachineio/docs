...
* Learn about connections in a circuit.
...
In electronics and telecommunication, electronic signals carry data from one device to another to send and receive all kinds of information. They are always time-varying, which means the voltage changes as time goes on. 

A Digital signal can have two states: on or off. The states are determined by the voltage values. The digital pins on the SwiftIO Feather have voltage values of either 3.3 volts (high) or 0 volts (low).
...
You can use a digital output to control the LED both built onto the board or external LEDs (not included). When you apply a high signal to the LED, it will turn on, and if you apply a low signal, it will be off. NOTE: sometimes a high signal can mean `false` while a low signal means `true`! It all depends on your circuit. 
...
Note: How to identify the two legs of LED?
...
Pin is the digital output pin, R is a resistor, is the LED, and GND is ground
...
Vcc is a power source, R is a resistor,  is the LED, and pin is the digital output pin
