...
We’ll talk about the sine wave in following part and only deal with periodic waveforms. Sine wave is the fundamental waveform. Other period waveforms can be divided into several different sine waves. 
...
**Sample depth** corresponds to the number of possible values between the minimum and maximum sound. The common choices are 16, 24, 32-bit. The higher the sample depth, the more audio data will be stored, so the original sound will be more likely to be reproduced.

When you wear earphones to listen to music, you can hear sounds in your left and right ear. The L and R are used respectively for the **left and right channels**. If only one channel is sampled, the sound is **mono**. **Stereo** means the audio has multiple channels, ususully the right and left channel, and thus needs two speakers.

As mentioned before, the maximum frequency for human hearing is about 20KHz, the sample rate should be double to recreate it, that is, about 40KHz. The CD audio is usually sampled at 44,100Hz, in other words, 44100 samples per second, and thus could cover almost all sounds humans can hear. What's more, CD-quality audio has a sample depth of 16-bit and has 2 channels. 

**Audio format**

Maybe you have heard of some audio file formats: MP3, WAV, AAC, FLAC, and so on. These formats can be divided into two types: lossless file format and lossy file format. The difference is the way all audio data is stored. For example, WAV and FLAC are lossless, while MP3 and AAC are lossy. Let’s have a look at the two common formats: WAV and MP3.

* **WAV** (or WAVE) file stores the raw PCM data. But others don't know how sound is sampled given only the data. So WAV file has a header at the beginning which tells the sampling information. This format stores all original audio data, thus it is large in file size. Beside, it doesn’t require any decoding method when open the file.
...
The working principle of the speaker is similar to the buzzer. When the current flows, the magnet filed generate in the circuit cause the internal diaphragm move back and forth, thus causing the air vibration. The air molecules vibrate and bump into each other, which leads to the sound you hear. 
...
while true {
    ...
    // The sound waveform is square wave, so you will hear buzzing sound.
    ...
    // Iterate through the frequencies from C to B to play a scale.
    // The sound waveform is triangle wave, and the sound is much softer.
