...
## Comparison between computer and MCU
...
- CPU
- RAM: is a type of volatile memory, meaning that its contents are lost when the power is turned off. It's used by the program to run, containing variables allocated by your program and data gathered from peripherals.
- ROM: it is non-volatile, which means whatever is written to it stays when there is no power to the device. It stores the firmware/bootloader responsible for performing basic initialization and configuration tasks, and for loading the main program from another source. It normally cannot be changed or modified.
- peripherals: additional circuits and components that are built into the MCU to provide specific functions. They expand the capabilities of the MCU and allow it to interact with the outside world. The common peripherals are GPIO, ADC, I2C, SPI, UART, Timer...

A **general-purpose computer** typically includes: 
- CPU
- RAM 
- ROM: typically stores the BIOS, firmware that controls the basic functions of the computer and provides instructions for initializing the hardware components of the computer. It is installed on the motherboard. A BIOS runs before, or instead of, the operating system. It can control any display device attached to the processor, and any storage attached (such as a disk drive), and any input device attached as well.
- peripheral interfaces: USB, Ethernet, Wi-Fi, Bluetooth, and more which are much faster compared to those on MCUs. These are used to interface with a wide range of external devices, from keyboards and mice to printers and external hard drives. 
