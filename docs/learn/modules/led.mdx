...
The resistor functions as a current-limiting component that, just as its name suggests, can resist the current in the circuit. It has two legs. You can connect it in either direction as it is not polarized. Its ability to resist the current, called resistance, is measured in **ohm (Ω)**.
...
As stated previously, all digital pins on the SwiftIO Feather board output a high signal of 3.3V. If the resistance in the circuit is 330Ω, the current would be 3.3 / 330 = 0.01A. 
...
* In a **series circuit**, devices are connected end-to-end in a chain, like R1 and R2. The current flows through them in one direction from positive to negative. And the current flowing through each device is the same.
  ...
  Suppose R1 is 100Ω and R2 is 200Ω, the voltage supply is 3V, 
...
* In a **parallel circuit**, devices share two common nodes, like R3 and R4. Node a connects both two devices, so the current flows through both of them. The voltage between two nodes (a and b) is the same.
  ...
  Suppose R3 is 100Ω and R4 is 200Ω, the voltage supply is 3V, 
...
On your first try, let's make the LED blink - on for one second, then off for one second, and repeat it over and over again.
...
In your everyday life, you must have used timers. For example, you are cooking meals, and the dish still needs another 30min. You can set a timer for 30min and do other work until the timer reminds you after 30min.

The timer here is quite similar. It is hardware built on the microcontroller. You can set an expected time before executing the task, called ISR. In this way, the microcontroller can do many other works before that. When the set time has elapsed, the interrupt occurs, and it will go to execute the ISR. After finishing, it goes back to continue its previous work. It's one of the interrupt mechanisms. And you will know more about interrupt in the next tutorial.
