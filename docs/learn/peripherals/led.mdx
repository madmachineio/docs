...
Digital signals are ideal for working with components that have discrete states, such as LEDs (on/off) and buttons (pressed or released).
...
**GPIO pins** can be configured as either **input or output pins using software**, meaning they can either read or send signals to control the state of an external device or to read data from an external sensor. 
...
- When a GPIO pin is configured as an [**input**](./button), it can read the voltage level of an external signal, such as the state of a button or the output of a sensor.
...
There are many types of LEDs with different colors and sizes. The LED on your SwiftIO Playground is a small variant designed to be convenient for mass production. You could see [this article](https://lastminuteengineers.com/light-emitting-diode-led/) for more details on LED.

### Resistor

The resistor functions as a current-limiting component that, just as its name suggests, can **resist the current** in the circuit. It has two legs. You can connect it in either direction as it is not polarized. 
<img
...
Its ability to resist the current, called **resistance**, is measured in **ohm (Ω)**.
...
As stated previously, all digital pins on the SwiftIO Feather board output a high signal of 3.3V. For example, if the resistance in the circuit is 330Ω, the current would be 3.3 / 330 = 0.01A. 
...
<details>
  ...
  * R = R<sub>R1</sub> + R<sub>R2</sub> = 300Ω
...
* In a **parallel circuit**, devices share two common nodes, like R3 and R4. Node a connects both two devices, so the current flows through both of them. The voltages between two devices are the same.
...
The LED is connected to **D18**. GND and 3V3 are connected respectively to the corresponding pin on the board. 
...
* you need to specify which pin is used. `Id` is an enumeration including the ids of all pins in `MadBoard`. As for **enumeration**, enum for short, it groups a set of related values. Just remember that the id needs to be written as `Id.D18`. 
* the `mode` of the digital pin is `pushPull` in most cases and you will know more about it in the future.
* the `value` decides the output state of the pin after it's initialized. By default, it outputs a low level. If you want the pin to output 3.3V by default, the statement should be `let led = DigitalOut(Id.D18, value: true)`.
...
This statement makes the pin D18 output high voltage. And the LED connected to that pin requires a high level to be on. Therefore, the LED turns on.
...
Configuration! You've finished all the projects🥳. 
