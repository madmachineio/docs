...
The timer here is quite similar. It is a software timer provided by Zephyr. You can set an expected time before executing a specified task. In this way, the microcontroller can do many other works before that. When the set time has elapsed, MCU will stop the current work and it will go to execute the task. After finishing, it goes back to continue its previous work.
