...
There is a random password that consists of several movement directions. You will need to move/tilt the board in the corresponding directions (left/right/forward/backward) to match the password within a given time. If you succeed, the red LED will turn on. You can view the message on a serial monitor as a notification.
